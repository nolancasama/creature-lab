...
| *(no failure path anywhere)* | Scaffold ladder: retry → "I heard *It was small*, now say *Now it is big*" → TTS models the sentence → the third real try is accepted whatever it sounded like | The success path was fully specified and the failure path was not. In a classroom the failure path decides whether a child keeps going. Nothing can dead-end a lesson. |
| *(the last rung used to be a button a teacher pressed)* | The game accepts the third attempt itself, and records it as assisted for the round report | Right idea, wrong hands: it stalled a child in front of the class until an adult walked over, and one teacher cannot walk to thirty machines. The child still does the work — three genuine spoken tries — and the teacher still learns which passes were helped. Only utterances that carry a frame word or an adjective count toward those three, so a cough or the next table's conversation cannot buy a pass for a child who never spoke. |
| "Easy Mode is the default gameplay mode" (no other mode defined) | Split into three independent teacher dials: **who chooses** (free/guided), **pronunciation tolerance** (lenient/standard/strict), **how much is printed** (whole sentence / frame with gaps / nothing) | One undefined word became three settings a teacher can actually raise over a term. Every tolerance still requires `was` / `is`; Hidden mode is where the student stops reading and starts producing. |
